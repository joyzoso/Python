...
def canvas_click(event):
    #these methods translates the xy coordinates to where they actually appear on the canvas
    x = canvas.canvasx(event.x)
    y = canvas.canvasy(event.y)
    canvas.create_oval((x-5, y-5, x+5, y+5), fill = 'green')

#calls the bind method on the widget and binds it to whenever the user clicks
